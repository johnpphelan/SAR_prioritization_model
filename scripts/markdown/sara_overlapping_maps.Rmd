...
### as things are on onedrive and the path is long, set a new temp path
# tempdir_local <- "C:/Temp/R_temp"
# dir.create(tempdir_local, showWarnings = FALSE, recursive = TRUE)
# Sys.setenv(TMPDIR = tempdir_local)


```

```{r grab_Data_and_geoms}
setwd(here::here())
source(here::here("scripts","utils","load_data.R"))
source(here::here("scripts","utils","prep_predictor_data_f.R"))
...
dfo_sar_w_wb_no_dups = dfo_sar_w_wb |>
  # Pull geometry out separately (one row per waterbody group)
  dplyr::mutate(Population_EN = tidyr::replace_na(Population_EN, "NA")) |>
  { \(d) {
    # 1. Do the slow text summarise on a plain data frame (no geometry overhead)
    summary_df = sf::st_drop_geometry(d) |>
      dplyr::group_by(waterbody, BLUE_LINE_KEY, watershed, FWA_WATERSHED_CODE, wb_type) |>
      dplyr::summarise(
        Common_Name_EN  = paste0(Common_Name_EN, collapse = ', '),
        Population_EN   = paste0(na.omit(Population_EN), collapse = ', '),
        Scientific_Name = paste0(Scientific_Name, collapse = ', '),
        .groups = 'drop'
      )

    # 2. Get one geometry per group (just take the first — they should be the
    #    same waterbody polygon repeated across SAR rows)
    geom_df = d |>
      dplyr::group_by(waterbody, BLUE_LINE_KEY, watershed, FWA_WATERSHED_CODE, wb_type) |>
      dplyr::slice(1) |>
      dplyr::select(waterbody, BLUE_LINE_KEY, watershed, FWA_WATERSHED_CODE, wb_type)

    # 3. Rejoin
    dplyr::left_join(geom_df, summary_df,
                     by = c("waterbody", "BLUE_LINE_KEY", "watershed",
                            "FWA_WATERSHED_CODE", "wb_type"))
  }}()
...
ais_occs_no_dups = ais_occs_w_wb |>
  dplyr::group_by(waterbody, BLUE_LINE_KEY, watershed, FWA_WATERSHED_CODE, wb_type) |>
...
for(i in 1:nrow(dfo_sar_w_ais)){
  ...
  for (ais_spp in sp_upstream_of_wb_split) {
    ...
    if(!file.exists(paste0(species_folder,"MaxEnt_prediction_habitat_or_not.tif"))){
      ...
      #waterbody only
      the_pred_wb_masked <- terra::mask(the_pred_about_wb, the_wb)
...
ais_effects_on_fish = read_excel(here::here("data","ais_effects.xlsx"), sheet = 'fish')
ais_effects_on_inverts = read_excel(here::here("data","ais_effects.xlsx"), sheet = 'invertebrates')
...
source(here::here("scripts","utils","sar_risk_merge.R"))
...
dfo_output = dfo_output_long |>
  ...
  dplyr::group_by(waterbody, watershed, Common_Name_EN, Population_EN, Scientific_Name,
                  ...
                  number_upstream_waterbodies, mean_of_maxent_hab_not_hab, FN_noted,
...
dfo_output_final = dfo_output_final |>
  ...
  dplyr::select(waterbody, watershed, Common_Name_EN, Population_EN, COSEWIC.status,
                ...
                FN_noted, expert_opinion_AIS, expert_opinion_SARA,
...
dfo_output_final = dfo_output_final |>
  dplyr::left_join(
    dfo_sar_w_wb_no_dups |>
      dplyr::select(waterbody, BLUE_LINE_KEY, watershed, FWA_WATERSHED_CODE, wb_type),
    by = c("waterbody", "watershed")
  )

## we have the blue line keys and the fwa watershed codes - lets get geometries from bcdc for each of them

wb_keys = dfo_output_final |>
  dplyr::distinct(waterbody, BLUE_LINE_KEY, FWA_WATERSHED_CODE, wb_type)

# Split by type since lakes and rivers are in different layers
lake_keys = wb_keys |> dplyr::filter(wb_type == "lake")
river_keys = wb_keys |> dplyr::filter(wb_type == "river")

# Fetch lake geometries
lake_geoms = bcdata::bcdc_query_geodata('freshwater-atlas-lakes') |>
  bcdata::filter(BLUE_LINE_KEY %in% lake_keys$BLUE_LINE_KEY) |>
  bcdata::collect() |>
  sf::st_transform(4326) |>
  dplyr::select(BLUE_LINE_KEY, FWA_WATERSHED_CODE) |>
  dplyr::group_by(BLUE_LINE_KEY, FWA_WATERSHED_CODE) |>
  dplyr::summarise(.groups = 'drop')  # unions multi-part polygons per waterbody

# Fetch river geometries
river_geoms = bcdata::bcdc_query_geodata('freshwater-atlas-rivers') |>
  bcdata::filter(BLUE_LINE_KEY %in% river_keys$BLUE_LINE_KEY) |>
  bcdata::collect() |>
  sf::st_transform(4326) |>
  dplyr::select(BLUE_LINE_KEY, FWA_WATERSHED_CODE) |>
  dplyr::group_by(BLUE_LINE_KEY, FWA_WATERSHED_CODE) |>
...
# Combine and join back
all_geoms = dplyr::bind_rows(lake_geoms, river_geoms)

dfo_output_final = dfo_output_final |>
  dplyr::left_join(all_geoms, by = c("BLUE_LINE_KEY", "FWA_WATERSHED_CODE")) |>
  sf::st_as_sf()

```

## Mapping Binned Risk {.tabset}

### Risk bins by waterbody - All species

```{r spatial_manipulations}
# now we should group by waterbody - plot the highest risk factor?
bc <- bcmaps::bc_bound() |> st_transform(4326)

test <- dfo_output_final |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 

pal <- colorFactor(
  palette = c("darkgreen", "purple", "brown"),
  levels  = c(1, 2, 3)
)
...
leaflet() %>%
  ...
  addPolygons(
    data        = test,
    fillColor   = ~pal(final_risk_b),   # use pal() instead of case_when
    color       = ~pal(final_risk_b),
    weight      = 1,
    fillOpacity = 0.7,
    popup       = ~paste0(
  ...
  addLegend(
    ...
    pal       = pal,
    values    = test$final_risk_b,
    title     = "Final Risk Level",
    labFormat = labelFormat(prefix = "Risk ")
  )

```

### Binned Risk - Bull Trout

```{r first_sp_map}
test <- dfo_output_final |> 
  filter(Common_Name_EN == "Bull Trout") |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 
...
```{r}
test <- dfo_output_final |> 
  filter(Common_Name_EN == "Westslope Cutthroat Trout") |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 
...
### Binned Risk - Sockeye Salmon

```{r}
test <- dfo_output_final |> 
  filter(Common_Name_EN == "Sockeye Salmon") |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 
...
### Binned Risk - Cultus Pygmy Sculpin

```{r}
test <- dfo_output_final |> 
  filter(Common_Name_EN == "Cultus Pygmy Sculpin") |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 
...
### Binned Risk - Rocky Mountain Ridged Mussel

```{r}
test <- dfo_output_final |> 
  filter(Common_Name_EN == "Rocky Mountain Ridged Mussel") |> 
  dplyr::group_by(waterbody) |>
  dplyr::slice_max(final_risk_b, n = 1, with_ties = FALSE) |>
  dplyr::ungroup() 
