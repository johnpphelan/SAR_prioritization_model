...
dfo_output_long <- dfo_output_long |>
  ...
  dplyr::mutate(
    sum_ais_effect = sum(value, na.rm = TRUE),
    sum_ais_effect_count = sum(!is.na(value))
  ) |>
  dplyr::ungroup() |>
  tidyr::pivot_wider()
...
dfo_output = dfo_output_long |>
  dplyr::mutate(ais_sp_in_wb_summed_effect = paste0(ais_present_names, " - ", round(sum_ais_effect,3)," (Uncert. ",uncertainty,")")) |>
  ...
  dplyr::reframe(across(c(ais_present_names,ais_upstream_names,ais_sp_in_wb_summed_effect), \(x) paste0(unique(x), collapse = ', ')),
                 summed_ais_in_wb_effects_total = sum(sum_ais_effect,na.rm=T),
                 summed_ais_in_wb_effects_count_total = sum(sum_ais_effect_count, na.rm = T),
...
dfo_output_long = dfo_output_long |>
  ...
  mutate(summed_ais_upstream_effect = sum(value,na.rm=T),
         sum_ais_effect_upstream_count = sum(!is.na(value))) |>
  dplyr::ungroup() |>
  tidyr::pivot_wider()

# Resummarise by waterbody and SARA species!
dfo_output_upstream = dfo_output_long |>
  dplyr::mutate(upstream_ais_sp_summed_effect = paste0(ais_upstream_names, " - ", round(summed_ais_upstream_effect,3)," (Uncert. ",uncertainty,")")) |>
  dplyr::group_by(waterbody, watershed, Common_Name_EN, Population_EN, Scientific_Name,
                  ...
                  summed_ais_in_wb_effects_total, summed_ais_in_wb_effects_count_total, summed_in_wb_uncertainties, mean_in_wb_uncertainty,
                  COSEWIC.status) |>
  dplyr::reframe(across(c(ais_upstream_names,upstream_ais_sp_summed_effect), \(x) paste0(unique(x), collapse = ', ')),
                 summed_upstream_ais_effects_total = sum(summed_ais_upstream_effect,na.rm=T),
                 summed_ais_upstream_effects_count_total = sum(sum_ais_effect_upstream_count, na.rm = T),
...
dfo_output_final = dfo_output |>
  ...
  dplyr::left_join(
    dfo_output_upstream |> dplyr::select(waterbody,watershed,Common_Name_EN,ais_present_names,
                                         summed_upstream_ais_effects_total:mean_upstream_uncertainty)
  )

dfo_output_final = dfo_output_final |> 
  mutate(summed_risk_in_wb = summed_ais_in_wb_effects_total / summed_ais_in_wb_effects_count_total,
         summed_risk_upstream = summed_upstream_ais_effects_total / summed_ais_upstream_effects_count_total)
...
dfo_output_final = dfo_output_final |>
  ...
  dplyr::mutate(
    summed_ais_ef_in_wb_on_status = summed_risk_in_wb + cosewic_numeric,
    summed_ais_ef_upstream_on_status = 0.5 * (summed_risk_upstream + cosewic_numeric)
  ) |>
  dplyr::mutate(summed_ais_ef_in_wb_on_status = tidyr::replace_na(summed_ais_ef_in_wb_on_status, 0),
                summed_ais_ef_upstream_on_status = tidyr::replace_na(summed_ais_ef_upstream_on_status, 0)) #|>
...
dfo_output_final = dfo_output_final |>
  mutate(final_risk = summed_ais_ef_in_wb_on_status + summed_ais_ef_upstream_on_status)
# dfo_output_final = dfo_output_final |>
...
dfo_output_final = dfo_output_final |>
  ...
  dplyr::select(waterbody, watershed, Common_Name_EN, Population_EN, COSEWIC.status,
                ...
                summed_ais_in_wb_effects_total, summed_ais_in_wb_effects_count_total,
                #summed_ais_in_wb_effects, summed_in_wb_uncertainties,
                # Sum of effecs of AIS upstream
                summed_upstream_ais_effects_total, summed_ais_upstream_effects_count_total,
...
openxlsx::addWorksheet(my_wb, "habitat_suitabilities")
openxlsx::writeData(my_wb, "habitat_suitabilities", hab_suit_df)
openxlsx::saveWorkbook(my_wb, paste0("output/Summed_SARA_prioritization_model_",Sys.Date(),".xlsx"), overwrite = T)
