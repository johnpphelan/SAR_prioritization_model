...
```{r setup, echo = F, include=FALSE, results = 'asis', message = F, warning = F}
...
Columbia. The overlap is found based on named rivers, streams, and lakes within the BC Freshwater Atlas. The locations of the SAR are represented by polygons (and points for the Sockeye salmon in Cultus Lake).

Risk assessments and AIS Alert Sheets provide detail on potential impacts of AIS on SAR, and the uncertainty associated with these impacts.
...
The waterbodies with SAR and AIS overlapping are presented below. Hwoever, this does not include those that have AIS upstream. This is presented in a later plot.

```{r ais_sar_overlap}

filted_sar_ais<- filtered_wbs[sf::st_intersects(filtered_wbs, ais_occs, sparse = FALSE) |> rowSums() > 0, ]
  
ggplot()+
  geom_sf(data = st_as_sf(bc))+
  geom_sf(data = filted_sar_ais, fill = "steelblue", alpha = 0.5) +
  geom_sf(data = frascol, fill = NA, color = "black", linewidth = 0.75) +
  ggthemes::theme_map()
...
When considering upstream connections, to visualise the connections, it may be necessary to look at un-named waterbodies to visualise these connections.
```{r upstream_example}
...
These five categories are scored from low (1) to high (3) impact. For each species, the impact on a SAR is averaged across the above categories.
